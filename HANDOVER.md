...
这是一个基于 Microsoft **Qlib** 框架开发的 A 股日频量化投资系统，目标是从纯研究走向实盘。当前阶段：**Qlib/影子共享执行规划器和379日历史对账已通过，完整季度模型回填待 23:00 后执行**。未来 QMT 必须复用同一 T-1 信号包、执行日过滤补选、先卖后买和实际现金重算规则。
...
- ✅ 新增 `trade_planner.py`、不可变 `SignalPackage`、影子先卖后买状态机和 `qlib_adapter.py`；Qlib/影子共用 T-1 Top100、执行日过滤补选、实际卖后现金重算和0.3%价格保护
- ✅ 2025-01-02~2026-07-28 共379日严格同成本控制：订单和持仓逐日100%一致、首次分叉为空、规划订单差异0，逻辑现金最大差小于0.05元
- ✅ 严格控制期末 Qlib 138,072.07、影子135,343.41；剩余差额是复权估值与原始价/停牌缓存估值口径，不影响订单和持仓
- ✅ 生产成本语义 Qlib期末138,069.13、影子128,492.62，订单匹配80.08%、持仓匹配83.19%；Qlib成交量相关 impact 成本与影子固定滑点会跨过整手边界并产生可解释路径差
- ✅ 最终报告：`my/artifacts/shadow_backtest_parity/shared-planner-v3-20250102-20260728/report.md`

### 还没做（Codex 接手的第一优先级）
1. **加停牌/退市风险防护**：候选池排除ST/退市/长停票并补账户告警
2. **23:00 后训练 2026Q3 模型**：直接跑 backfill 会自动触发并保存到 `my/quant_state/models/2026Q3.txt`
3. **完整真实模型回填验证**：选 2026-07-20 ~ 2026-08-01 跑
   ```bash
   .venv/bin/python my/scripts/shadow_run.py backfill 2026-07-20 2026-08-01
   ```
4. 核对模型实时打分版的 orders/receipts/state/nav、涨跌停/停牌/重试日志及净值连续性
5. 完整回填通过后再启用每日调度；当前不要把局部烟测当成正式影子账户

### 回填验证中需要重点排查的潜在 bug
- **生产成本差异**：Qlib 的 `impact_cost` 与影子/QMT实际成交滑点语义不同，允许现金和净值产生可解释偏离；严格同成本控制用于验收规划逻辑。
