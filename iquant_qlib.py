...
ACCOUNT_TYPE = "stock"        # FUTURE/STOCK/CREDIT/HUGANGTONG/SHENGANGTONG/STOCK_OPTION
POSITION_STRATEGY_NAME = ""   # 若不区分策略，可置空
...
def _fetch_positions(ContextInfo):
    ...
    if acc_id:
        try:
            ...
            if data:
                print("[DEBUG] fetch holdings via get_trade_detail_data")
                return data
        except Exception as err:
            print(f"[WARN] get_trade_detail_data failed: {err}")
    for name in ("get_positions", "get_position", "query_positions", "get_position_info"):
        func = getattr(ContextInfo, name, None)
        if callable(func):
            try:
                data = func()
                if data:
                    print(f"[DEBUG] fetch holdings via {name}")
                    return data
            except Exception as err:
                print(f"[WARN] fetch holdings via {name} failed: {err}")
    return None
...
def _convert_positions(raw):
    ...
    for item in records:
        ...
        position = item.get("position") or item.get("volume") or item.get("qty") or item.get("amount")
        available = item.get("available") or item.get("enable_amount") or item.get("enable_volume")
        cost = item.get("cost_price") or item.get("avg_price") or item.get("price")
...
def _fetch_quotes(ContextInfo, symbols):
    full_tick = getattr(ContextInfo, "get_full_tick", None)
    if callable(full_tick) and symbols:
        try:
            data = full_tick(symbols)
            if data:
                print("[DEBUG] fetch quotes via get_full_tick")
                return data
        except Exception as err:
            print(f"[WARN] get_full_tick failed: {err}")
    for name in ("get_quotes", "get_last_quotes", "query_quotes", "get_realtime_quotes"):
        func = getattr(ContextInfo, name, None)
        if callable(func):
            try:
                data = func(symbols) if symbols else func()
                if data:
                    print(f"[DEBUG] fetch quotes via {name}")
                    return data
            except Exception as err:
                print(f"[WARN] fetch quotes via {name} failed: {err}")
    return None
...
def _convert_quotes(raw, symbols):
    if isinstance(raw, pd.DataFrame):
        ...
        records = []
    symbol_set = {_normalize_code(s) for s in symbols} if symbols else set()
    rows = []
    for item in records:
        code = item.get("code") or item.get("stock") or item.get("symbol") or item.get("m_strInstrumentID")
        if code is None:
        ...
        if symbol_set and ncode not in symbol_set:
            continue
        last = (
            item.get("last")
        ...
        bid_arr = item.get("bidPrice") or item.get("bidprice") or item.get("bid_prices")
        ask_arr = item.get("askPrice") or item.get("askprice") or item.get("ask_prices")
        bid1 = item.get("bid1") or item.get("bid") or item.get("m_dBidPrice1")
        ask1 = item.get("ask1") or item.get("ask") or item.get("m_dAskPrice1")
        if bid1 in (None, "") and isinstance(bid_arr, (list, tuple)) and bid_arr:
            bid1 = bid_arr[0]
        if ask1 in (None, "") and isinstance(ask_arr, (list, tuple)) and ask_arr:
            ask1 = ask_arr[0]
        high_limit = item.get("high_limit") or item.get("up_limit") or item.get("upper_limit")
        low_limit = item.get("low_limit") or item.get("down_limit") or item.get("lower_limit")
        rows.append(
            {
                "code": ncode,
...
def handlebar(ContextInfo):
    ...
    if phase != "orders_ready":
        print(f"[iQuant][INFO] state.phase={phase}，未达 orders_ready，跳过")
        return
    if TARGET_VERSION and version != TARGET_VERSION:
        print(f"[iQuant][INFO] state.version={version} ≠ 期望 {TARGET_VERSION}，跳过")
        return
    if _FIRED:
        print("[DEBUG] 已执行过下单，跳过")
        return
