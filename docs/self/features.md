...
## 月度 pos_ratio（实验开关，默认关闭）

### 功能说明

在 `TopkDropoutStrategy` 中新增了一个可选的月度仓位系数 `pos_ratio` 机制，用于按月缩放目标仓位规模，不改模型和选股排序逻辑。

- 开关参数：`enable_monthly_pos_ratio`（默认 `false`）
- 基准参数：`monthly_pos_ratio_benchmark`（当前用 `SH000300`）
- 状态文件：`monthly_pos_ratio_state_path`（JSON 持久化）

### 规则（已实现）

- 每月更新一次，下个月生效
- 使用最近两个月“带成本月超额收益”正负组合决定仓位
- 取值仅 `{1.0, 0.5, 0.1}`
- 冷启动（月数不足 2）时 `pos_ratio=1.0`

### 回测对照结果（同一模型，仅开关差异）

- 2025 测试段：`ON` 相比 `OFF`
- 含成本年化超额：`0.431233 -> 0.417326`
- 含成本 IR：`2.552304 -> 2.417839`
- 含成本最大回撤：`-0.099455 -> -0.105177`
- 2024 测试段：`ON` 相比 `OFF`
- 含成本年化超额：`-0.031337 -> -0.114377`
- 含成本 IR：`-0.133857 -> -0.505430`
- 含成本最大回撤：`-0.278479 -> -0.261746`（仅回撤略好）

### 当前结论

- 该功能保留为实验开关，不作为默认策略能力。
- 常规回测和手动实盘（manual）均保持关闭：`enable_monthly_pos_ratio: false`。
- 该功能不会影响其他 qlib 回测，只有显式打开开关才生效。
