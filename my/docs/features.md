...
**实现时间**: 2026-08-04 至 2026-08-05
**状态**: 🚧 核心、共享规划器和历史对账已验证，真实季度模型回填待执行
...
对账器将同一份 `candidate1_pred.pkl`、10万初始资金、SH000905 门控以及 TopK50/N_DROP2 参数同时送入 Qlib 适配器和影子两阶段执行，逐日比较净值、逻辑现金、原始股数持仓、规划订单、成交回执和候选跳过原因。每次运行输出：

对账器一次性缓存整段行情，逐日对比净值、现金、持仓、订单和影子回执。Qlib 内部的复权持仓数会用当日或最近有效 factor 还原为实际股数，可正确处理停牌/退市后仍在账的持仓。每次运行输出：
...
**验证**: 影子、规划器和对账相关测试全部通过；379日生产/严格控制报告可重复生成。
