...
**实现时间**: 2026-08-04
**状态**: 🚧 核心和历史对账已验证，真实季度模型回填待执行

新增 `my/quant/` 影子交易核心和 `my/scripts/shadow_run.py` 入口，覆盖数据新鲜度停摆、SH000905 门控、季度模型打分、TopK订单、开盘影子成交、持久化账本以及独立历史回填。当前共有29项影子核心/对账测试，并完成7个真实交易日的隔离回填烟测以及379个交易日的黑盒对账；正式启用步骤和产物说明见 `my/docs/shadow_mode.md`。
...
新增 `my/quant/parity.py` 和 `my/scripts/compare_shadow_backtest.py`，将同一份 `candidate1_pred.pkl`、10万初始资金、SH000905 门控以及 TopK50/N_DROP2 参数同时送入两条路径：

1. Qlib `backtest_daily` + `GatedTopkDropout`，保留 `only_tradable=True`。
2. 现有影子 `nightly.run_evening`，前一晚锁定股数，次日开盘不替补。
