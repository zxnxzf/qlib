...
日期：2026-08-14
状态：第一版本地实现完成，等待 Windows 国金标准QMT只读探针与预演
适用范围：国金标准 QMT 内置 Python；不使用 MiniQMT、XtQuant 外部接口

当前实现状态（2026-08-15）：协议、生产信号、只读能力探针、共享规划器、券商无关执行状态机、真实绩效账本和只读对账已完成本地自动测试。QMT部署入口目前硬锁为`read_only`，并只允许加载已审查的`GuojinQmtBridge`；国金模拟报单、查单、成交和撤单适配必须等Windows探针确认真实接口后再实现，不能仅靠修改配置开启。
...
| `expires_at` | 最晚有效时间，默认T日09:32（即09:30～09:31两个分钟内有效） |
...
```json
{
  ...
  "expires_at": "2026-08-17T09:32:00+08:00",
...
- 信号在09:32前未到、已经过期或数据日不正确：当天不交易，写`aborted`结果。
...
| `market_snapshot` | 卖出规划时实际使用的盘口和涨跌停信息 |
| `sell_stage` | 卖出规划、跳过原因、委托、成交和撤单 |
| `account_after_sell` | 卖单终态后的券商现金、持仓、证券市值和总资产 |
| `buy_market_snapshot` | 卖单终态后重新读取、供买入规划使用的盘口 |
...
为计算QMT真实收益，QMT在T日15:00或之后的实时bar再次查询券商账户，单独写入：
...
本地状态：只读结构探针已完成；Windows实测和后续最小模拟报单探针未完成。
...
- 探针报告只保存字段名、类型和结构，不保存真实账号、现金、持仓代码/数量或委托成交值。
- 若内置Python低于3.8，正式入口保持禁用；根据实测版本再决定兼容实现，不能绕过版本门禁。

验收：能够可靠查询账户/持仓/总资产/行情，并确认报单、查单、查成交、费用、撤单和收盘快照的准确接口。

### 阶段B：共享协议和规划器兼容

本地状态：已完成，包含不可变文件、校验码、过期/版本检查和共享规划器安全校验。最终379日严格零价格冲击复验中，订单/持仓明细和整日匹配率均为100%，规划订单与跳过原因差异均为0，逻辑现金最大差0.067642元；净值仍受残留不可卖持仓估值口径影响。
...
本地状态：已完成代码和自动测试，待Windows用真实T-1数据运行。
...
本地状态：券商无关状态机和假券商测试已完成；真实国金报单桥仍硬锁，必须等待探针。

- 实现预检、卖单、等待/撤单、账户刷新、买单、结果归档、收盘账户快照和崩溃恢复。
- 同一执行日使用PID/token感知的跨进程独占锁；传入信号仍须完整校验，旧批次结果不得冒充新批次。
- 券商委托、成交和撤单事实必须关联规划订单；重复成交ID、超额成交、代码或方向不一致全部硬失败。
- 先接假券商适配器跑自动测试，再接国金QMT模拟账户。
- 原 `iquant_qlib.py` 保留作参考，不直接修改为生产脚本。

### 阶段E：QMT真实绩效账本

本地状态：已完成券商事实账本、资金流、费用/滑点、HTML报告和幂等导入测试，待Windows真实字段验证。
...
- 无本地订单号的外部委托按券商ID或稳定摘要分别保留并标记异常，不合并、不静默丢弃。
...
- 同日双实例并发、同日不同批次、失效信号和旧结果批次错配全部拒绝执行。
- 券商回报中的未知订单、重复成交ID、超额成交、代码/方向不一致全部拒绝进入完成状态。
- QMT账户快照与本地成交推算不一致时，以券商为准并报警。
- 持仓查询为空但账户证券市值为正时硬失败，不按空仓继续规划。
...
本地结果（2026-08-15）：QMT协议、执行、绩效、共享规划器和Qlib适配器专项166项通过，完整`my/tests`共266项通过；379日证据目录为`my/artifacts/shadow-backtest-parity/qmt-planner-safety-final-20260815/strict_cost_control/`。
