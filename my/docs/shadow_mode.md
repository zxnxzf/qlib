...
**状态**：✅ 共享执行规划器、策略发布包、379日历史对账和2026Q3正式模型已通过；QMT仍按模拟/试用优先开发。
...
2026-08-14 重建并发布2026Q3原生Booster：2026-07-01至07-28归档域104,113条评分最大误差0，每日Top100最低重合100%；当前数据新增26行，完整当前数据Top100最低重合仍为100%。详细证据见策略包的 `releases/2026Q3-validation.md`。

## 正式启用前检查

1. 对ST/退市/长停票加入组前过滤和账户告警。
2. 用正式发布模型完整回填 2026-07-20~08-01，核对订单、回执、状态、净值和异常日志。
3. 完整回填通过后，再配置 20:30~次日08:00 的多轮调度与告警。
4. QMT 试用环境可用后，按标准QMT设计复用同一策略包和规划器，先跑模拟/试用，不直接连接实盘资金。
