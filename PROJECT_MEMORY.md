...
2026-08-04 同区间对账已完成（实现提交 `a5a99963`）：`my/quant/parity.py` + `my/scripts/compare_shadow_backtest.py` 使用同一 `candidate1_pred.pkl`、10万资金、SH000905门控、TopK50/N_DROP2 对2025-01-02~2026-07-28共379日做黑盒直比，2024-12-31预热。门控282个在场日/97个离场日完全一致；Qlib期末144,631.46（区间+44.63%、年化+26.08%、MDD -16.29%），影子期末121,248.55（区间+21.25%、年化+12.86%、MDD -24.22%）。首次分叉2025-01-15：影子当日6笔涨停禁买，Qlib `only_tradable=True` 会执行日过滤并补选，影子前夜锁定后不补选；股数还受“Qlib按当日开盘预算 vs 影子按前日收盘锁定”影响。影子回执 filled=1449、blocked_limit=97、suspended=229、no_data=230、insufficient_cash=6，说明回测执行假设明显乐观，不能直接当成可实现实盘绩效。处理Qlib复权持仓时发现并修复停牌/退市后当日缺factor的对账问题，改用最近有效factor并按100股整手消除日间factor微漂移假差异；最终订单差异分籺rounding_or_cost=487、execution_tradability=1941、initialization/unexplained=0。31项核心+对账测试通过，三次完整执行期末净值一致。最终报告为 `my/artifacts/shadow_backtest_parity/full-20250102-20260728-v3/report.md`。

## 下一步
1. 根据1.5年对账结果拍板 QMT/影子执行语义：建议前一晚输出排名候选与持仓单，执行日用实时行情过滤不可交易票、按当日价重算股数并从备选中补位；不建议继续把前夜锁定且不补选的影子数字与Qlib收益等同。
2. 在QMT前解决全市场票池的停牌/退市风险：对账期影子产生229笔suspended和230笔no_data卖出回执，期末仍有5只无法正常退出的持仓；需在候选池加ST/退市/长停过滤和账户告警。
3. 23:00 后运行 `shadow_run.py backfill 2026-07-20 2026-08-01`，自动训练并落盘2026Q3模型，完成真实模型打分端到端验证。
4. 真实模型回填与新执行语义通过后，配置每日20:30~次日8:00多轮调度与告警；不提交 `my/quant_state/` 运行账本。
5. 用户线下继续确认国金QMT权限、免5佣金与10万元资金；QMTExecutor/Syncthing等影子模式通过后再开发。
...
- 研究回测 `only_tradable=True` 与隔夜影子“次日只拒单不补选”的差异已被1.5年对账实证：年化收益相差13.22个百分点，回撤相差7.93个百分点，必须在QMT前拍板并重新验证执行语义。
- Qlib当前 `impact_cost` 是按成交额/市场成交量平方调整的成本，影子是成交价固定±0.1%；这也是绩效差的一部分，尚未统一。
...
- 回测/影子对账保留两条路径的真实语义，不为追求数字一致增加“兼容模式”；任何执行语义修改都需重跑379日对账。
