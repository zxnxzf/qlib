...
## 下一步
1. 主线候选（开盘执行+n_drop2+5日label+滚动，需≥50万资金）：已过长窗大考与决赛多科，剩 IR(1.00/1.5) 一座山——队列中 DoubleEnsemble/集成（提IR候选手段）与门控引擎级验证（MA20+3天确认定版）待跑。
2. 小资金线（用户的5万）：v3信号成立但净仅+1.0%不达标；方向级三选一待用户拍板：①继续深挖受限池 ②换资产类别(ETF轮动,需立项) ③5万当流程仓先跑熟链路。
3. 用户现实行动项：确认国信/国金佣金是否有5元/笔最低（免5价值=6.1pp/年）；国金开户与QMT/PTrade权限仍在办理链路上。
4. 拍板悬置：研究口径升级为含0.1%滑点（已事实采用于新实验，正式确认待用户）。
5. 执行链路（QMT迁移/影子模式）等策略侧产出合格候选后重启。
