...
更新: 2026-08-14 11:45
核对基线:
- qlib | branch codex/shadow-backtest-parity | HEAD 为本文件所在阶段提交（父提交 26a1e464）| 门控温度计实验、研究台账和Windows生产部署决策已提交；提交后工作区 clean；当前分支比 main ahead 18
...
## 现在做到哪
仓库根目录为 `/Users/bytedance/code/qlib`，当前分支 `codex/shadow-backtest-parity`，HEAD 为 `26a1e464`。局部 Python 环境仍使用 `/Users/bytedance/code/qlib/.venv`；个人量化工作已统一迁移到 `my/` 目录，旧 `.data`、`run-configs`、`runs/mlruns` 口径只作为早期跑通记录保留，不再代表当前主线状态。当前未提交工作区包含 `my/docs/research_log.md` 的门控温度计台账更新，以及两个未跟踪实验脚本 `my/scripts/exp_gate905.py`、`my/scripts/exp_gate_thermometer.py`。
...
2026-08-04 执行链路定案：QMT 部署在用户家里的 Windows 电脑（有人值守自动化形态：交易日早上保证开机+QMT登录，心跳告警，文件未到/客户端未活=当天不交易）；Mac↔Windows 文件桥=Syncthing 先行（互联网 P2P+中继，几KB CSV 秒级），不稳换坚果云，git 只做订单留痕不做传输；云VPS/PTrade 为将来彻底无人值守的升级路线。数据源审计（100 release/4月）：已发布者 0/100 晚于次日8:30，但缺发≈15/83 工作日（含7月连续5天断供）→影子模式统计停摆；实盘前必配免费指数备源救门控，Tushare 200元/年为全量备源候选。
...
## 下一步
1. 先形成Windows生产Qlib + 标准QMT内置执行脚本的决策完备开发计划，明确发布边界、本地文件协议、QMT生命周期、幂等、报撤单、回执和验收。
2. 在国金标准QMT模拟/试用环境做最小能力探针：读取本地目录、导入纯Python规划器、查询账户/持仓/盘口、报单、查询成交和撤单。
3. 23:00后生成真实 `2026Q3` 季度模型，并完成 2026-07-20~08-01 模型打分回填。
4. 加入ST/退市/长停过滤、异常持仓告警和免费指数备源，再启用正式影子每日调度。
5. QMT模拟链路实现后，在个人Windows本地延迟补跑影子模式并做规划订单、真实成交和账户结果对账。
...
- 生产部署采用“Mac研究、Windows生产”：公司Mac不持续接收或保存券商账户、委托和成交数据；策略定版时只人工发布代码、模型和配置到个人Windows，Windows本地完成数据更新、信号、影子、QMT执行和对账。
