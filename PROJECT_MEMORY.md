...
更新: 2026-08-05 15:51
核对基线:
- qlib | branch codex/shadow-backtest-parity | HEAD b35b7e96 | 共享规划器实现已提交；工作区仅有本记忆更新、既有研究台账修改和两个未跟踪实验脚本；当前分支比 main ahead 16
...
## 下一步
1. 23:00后生成真实 `2026Q3` 季度模型，并完成 2026-07-20~08-01 模型打分回填。
2. 加入ST/退市/长停过滤、异常持仓告警和免费指数备源，再启用正式影子每日调度。
3. 让统一语义的影子模式持续观察一段时间，核对每日订单、回执、现金、持仓和净值。
4. 国金QMT试用环境可用后实现 `ExecutionAdapter` 券商适配，先跑模拟/试用并与影子规划逐笔对账。
