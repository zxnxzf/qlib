...
更新: 2026-08-14 16:37
核对基线:
- qlib | branch codex/shadow-backtest-parity | HEAD 为本文件所在QMT设计文档提交（父提交 810b5af9）| 提交后工作区 clean；当前分支比 main ahead 19
...
## 现在做到哪
仓库根目录为 `/Users/bytedance/code/qlib`，当前分支 `codex/shadow-backtest-parity`；本轮QMT设计文档提交的父提交为 `810b5af9`。局部 Python 环境仍使用 `/Users/bytedance/code/qlib/.venv`；个人量化工作已统一迁移到 `my/` 目录，旧 `.data`、`run-configs`、`runs/mlruns` 口径只作为早期跑通记录保留，不再代表当前主线状态。本轮仅新增标准QMT接入设计文档并同步项目记忆，未修改交易代码。
...
2026-08-14 标准QMT接入设计已落到 `my/docs/specs/2026-08-14-standard-qmt-integration.md`：定义了Windows本地 `signal.json` 与 `result.json` 协议、基于现有 `iquant_qlib.py` 的QMT接口映射、T日先卖后买状态机、批次幂等与恢复、影子复用/隔离边界，以及能力探针→协议/规划器→生产信号→QMT执行→20日模拟盘的开发和验收顺序。部署节奏已改为Git发布代码到Windows、QMT每日独立执行、影子按5至10个交易日或半个月观察窗口集中回放；不使用Syncthing等持续文件同步。尚未修改交易代码。

## 下一步
1. 按 `my/docs/specs/2026-08-14-standard-qmt-integration.md` 先实现国金标准QMT能力探针，确认Python、文件、账户、持仓、盘口、报单、查成交和撤单接口。
2. 探针确认后实现 `signal.json` V2、QMT兼容纯规划器和Windows生产信号入口，并重跑379日严格对账。
3. 23:00后生成真实 `2026Q3` 季度模型，并完成 2026-07-20~08-01 模型打分回填。
4. 加入ST/退市/长停过滤、异常持仓告警和免费指数备源；影子模式保持按需回放，不启用每日调度。
5. QMT模拟链路实现后，先独立运行5至10个交易日，再在个人Windows集中回放该窗口并对比规划订单、真实成交和账户结果。
...
- 真实季度模型训练与完整模型打分回填尚未验证；归档预测+真实行情已完成7日烟测和379日对账。
...
- 生产部署采用“Mac研究、Windows生产”：Git是Mac到Windows的代码、配置和模型版本清单发布方式，Windows只拉取确认过的commit或tag；模型大文件可单次人工部署但必须用Git中的哈希清单核验。公司Mac不持续接收或保存券商账户、委托和成交数据，不使用Syncthing、网盘或共享目录同步运行文件。
